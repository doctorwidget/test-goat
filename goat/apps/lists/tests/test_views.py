...
class ListViewTest(TestCase):
    ...
    def test_displays_only_items_for_that_list(self):
        ...
        blist = List.objects.create()
        Item.objects.create(text='spam 1', list=blist)
        Item.objects.create(text='eggs 2', list=blist)

        response = self.client.get('/lists/%d/' % (alist.id,))

        self.assertContains(response, 'itemey 1')
        self.assertContains(response, 'itemey 2')
        self.assertNotContains(response, 'spam 1')
        self.assertNotContains(response, 'eggs 2')
    ...
    def test_can_save_a_POST_request_to_an_existing_list(self):
        ...
        self.client.post(
            ...
            data={'new_item_text': 'A new item for an existing list'}
        )
        ...
        self.assertNotEqual(other_list, correct_list)
...
class NewListTest(TestCase):
    ...
    def test_validation_errors_are_sent_back_to_home_page_template(self):
        response = self.client.post('/lists/new', data={'new_item_text': ''})
        self.assertEqual(response.status_code, 200)
        self.assertTemplateUsed(response, 'lists/home.html')
    ...
    def test_invalid_list_items_arent_saved(self):
        self.client.post('/lists/new', data={'new_item_text': ''})
